...
// --- Sortable Wrapper for your existing Thumbnails ---
...
export default function SplitTool() {
  ...
  const splitPages = async () => {
    ...
    try {
      ...
      setErrorPopup({ show: true, msg: (err as Error).message || "Split failed" });
            ...
            <div className="bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 rounded-2xl p-5 shadow-sm">
              ...
              {zipUrl && (
                <a
                  href={zipUrl}
                  download="split.zip"
                  className="mt-3 w-full py-3 bg-blue-600 text-white rounded-xl text-xs font-black flex items-center justify-center gap-2 animate-fade-in"
                >
                  <span className="material-symbols-outlined text-[18px]">
                    download
                  </span>
